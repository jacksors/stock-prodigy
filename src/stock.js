...
const API_KEY = "8ZBACJ1MXTUUCWLA";
  ...
  fetch(API_Call_History)
    ...
    .then((data) => {
      for (var date in data["Time Series (Daily)"]) {
        if (parseInt(date.substring(0, 4)) >= 2017) {
...
export const fetchStockLive = async (StockSymbol) => {
  let API_Call_Live = await fetch(
    `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${StockSymbol}&apikey=${API_KEY}`
  );
  return await API_Call_Live.json();
};
